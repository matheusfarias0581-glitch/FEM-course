...
c     --- ALTERADO: Passando materiais a(1) e tipos m(i1) ---
      call wvtk(a(1),m(i1),m(i2),m(i3),a(i4),a(i5),a(i6),nnode,
     .          numel,ndm,nen,ndf,nvtk)
      close(nvtk)
...
c     1. Imprimir Deslocamentos
      ...
      do 400 i = 1, nnode
        ...
        aux(3) = 0.0d0
        k1 = id(1,i)
        if (k1 .gt. 0) aux(1) = u(k1)
        k2 = id(2,i)
        if (k2 .gt. 0) aux(2) = u(k2)
...
c       Calculando a tensao EXATA no ponto de Gauss (sig = D * B * u)
        do 500 m = 1, 3
          ...
          do 450 n = 1, 8
            db(m,n) = 0.d0
            do 420 k = 1, 3
              db(m,n) = db(m,n) + dmat(m,k) * bmat(k,n)
  420       continue
